	...
	$.each(links,function(index, link) {
		if(link){
			...
			$.post( "http://localhost:3000/checkicon",data,function(res) {
				sortItems.render(res,sortItems.list[index]);
			});
		}else{
			sortItems.render(null,sortItems.list[index]);
		}
	});
};
...
SortItems.prototype.render = function(response,config){
	for (var i = 0; i < this.list.length; i++) {
		...
		if(response){
			if(response.icon===undefined){
				...
				elementHtml+=  `<td><img src="${response.icon}" class="iconItems" alt="" style="width: 100%;border-radius:15em;"></td>
								<td class="showItems-name">${response.name}</td>`;
			}
		}else{
...
SortItems.prototype.search = function(input) {
	console.log(input)
	var item = $('.fixcenter');
		item.hide();
		$.each(item,function(index, el) {
			var elValue = $(el).children('td.nameItems').text();
			var mInput = input.trim();
			if(elValue.indexOf(mInput) !== -1){
				$(el).fadeIn('slow');
			}else{
